...
import { bootResources, bootSequence, type BootResourceState, type BootStep } from '../data/bootSequence'
...
export function ClusterBoot({ onComplete }: { onComplete: () => void }) {
  ...
  return <main className="boot-screen" aria-label="Cluster boot sequence">
    ...
    <section className="boot-layout">
      ...
      <div className="boot-cluster" aria-live="polite"><div className="boot-cluster__top"><div><span className="eyebrow">Cluster</span><strong>zaheer-platform</strong></div><span className={`boot-state boot-state--${activeStep.state}`}>{activeStep.phase}</span></div><div className="boot-services">{systems.map((system) => <div key={system.label}><span className={`resource-dot resource-dot--${system.state}`} />{system.label}<small>{system.state === 'queued' ? 'Waiting' : 'Ready'}</small></div>)}</div><div className="boot-nodes"><p className="eyebrow">Worker nodes</p>{['control-plane-01', 'workload-devops-01', 'workload-sre-01'].map((node, index) => { const state = stepIndex < 5 + index ? 'not-ready' : stepIndex < 7 + index ? 'initializing' : 'ready'; return <div key={node}><span className={`resource-dot resource-dot--${state}`} /><strong>{node}</strong><small>{state === 'not-ready' ? 'NotReady' : state === 'initializing' ? 'Initializing' : 'Ready'}</small></div> })}</div><div className="boot-workload"><span className={`resource-dot resource-dot--${stepIndex < 10 ? 'pending' : stepIndex < 12 ? 'creating' : 'running'}`} /><div><span>portfolio-workloads</span><small>{stepIndex < 10 ? 'Pending' : stepIndex < 12 ? 'ContainerCreating' : 'Running'}</small></div><b>{stepIndex < 10 ? '0/5' : stepIndex < 12 ? '1/5' : '5/5'}</b></div></div>
